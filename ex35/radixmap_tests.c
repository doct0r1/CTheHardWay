...
static int test_search(RadixMap *map)
{
	...
	for (i = (unsigned int) (map->end / 2); i < map->end; i++) {
		d = &map->contents[i];
...
static char *test_operations()
{
	...
	mu_assert(map != NULL, "Failed tp make a map.");
	...
	while (map->end > 0) {
		...
		// test that the end is now the old value, but uint_32 max so it trails off.
