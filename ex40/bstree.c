...
static inline void BSTree_setnode(BSTree *map, BSTreeNode *node, void *key, void *data)
{
    ...
    if (cmp <= 0) {
        ...
        if (node->right) {
            ...
            node->right = BSTree_create(node, key, data);
        }
    }
}
...
static inline int BSTree_traverse_nodes(BSTreeNode *node, BSTree_traverse_cb travers_cb)
{
    int rc = 0;

    if (node->left) {
        rc = BSTree_traverse_nodes(node->left, travers_cb);
        if (rc != 0)
            return rc;
    }

    if (node->right) {
        rc = BSTree_traverse_nodes(node->right, travers_cb);
        if (rc != 0)
            return rc;
    }

    return travers_cb(node);
}

int BSTree_traverse(BSTree *map, BSTree_traverse_cb travers_cb)
{
    if (map->root) {
        return BSTree_traverse_nodes(map->root, travers_cb);
    }
...
static inline BSTreeNode *BSTree_node_delete(BSTree *map, BSTreeNode *node, void *key)
{
    ...
    if (cmp < 0) {
        ...
        if (node->left && node->right) {
            ...
            // finally it's swapped, so return seccessor instead of node
            return successor;
        } else if (node->left) {
