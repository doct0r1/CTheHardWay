...
char *test_get_set()
{
    ...
    mu_assert(result == &expect3, "Wrong to set &test3");
...
char *test_traverse()
{
    ...
    mu_assert(travers_called == 3, "Wrong count traverse!");

    traverse_called = 0;
    rc = BSTree_traverse(map, traverse_fail_cb);
    mu_assert(rc == 1, "Failed to travers!");
    mu_assert(travers_called == 2, "Wrong count traverse for fail!");
...
char *test_delete()
{
    ...
    bstring result = BSTree_get(map, &test);
