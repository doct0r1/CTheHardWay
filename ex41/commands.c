...
int Command_depends(apr_pool_t *p, const char *path)
{
	...
	for (line = bgets((bNgetc) fgetc, in, '\n'); line != NULL; line = bgets((bNgetc) fgets, in, '\n')) {
		btrimws(line);
...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
	...
	if (apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
		...
		rv = apr_dir_make(BUILD_DIR, APR_UREAD | APR_UWRITE | APR_UEXECUTE, p);
	...
	// indicates that an install to actually run
	return 1;
error:
	return -1;
}
...
int Command_build(apr_pool_t *p, const char *url, const char *configure_opts, const char *make_opts, const char *install_opts)
{
	...
	check(rc == 0, "Failed to cleanup after builed.");
