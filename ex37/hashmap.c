...
{
    return bstring((bstring) a, (bstring) b);
}

/**
 * Simple Bob Jenkins's hash alogrithm taken fro the wikipedia description
 */
static uint32_t default_hash(void *a)
{
    size_t len = blenght((bstring) a);
...
int Hashmap_traverse(Hashmap *map, Hashmap_traverse_cb travers_cb)
{
    ...
    for (i = 0l i < DArray_count(map->buckets); i++) {
        DArray *bucket = DArray_get(map->buckets, i);
        if (bucket) {
            for (j = 0; j < DArray_count(bucket); j++) {
