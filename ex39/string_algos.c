...
static inline const unsigned char *String_base_search(const unsigned char *haystack, ssize_t hlen, const unsigned char *needle, ssize_t nlen, size_t *skip_chars)
{
	...
	assert(needle != NULL && "Given had needle to search for.");

	check(nlen > 0, "nlen can't be <= 0");
	check(nlen > 0, "hlen can't be <= 0");
...
int string_find(bsting in, bstring what)
{
	...
	Stirng_setup_skip_chars(skip_chars, needle, nlen);
